...
let cardList = huntingList["cardData"];
...
const pageCount = Math.ceil(cardList.length / paginationLimit);
...
const setCurrentPage = (pageNum) => {
  ...
  for (let idx = prevRange; idx < currRange; idx++) {
    if (idx >= cardList.length) break;
    const card = document.createElement("div");
    card.className = "card col-xl-3 col-sm-12 col-md-6";
    const gallery = document.createElement("div");
    gallery.className = "gallery";
    const img = document.createElement("img");
    img.setAttribute("src", cardList[idx]["url"]);
    img.setAttribute("alt", cardList[idx]["name"]);
    gallery.appendChild(img);
    const desc = document.createElement("div");
    desc.className = "desc";
    const name = document.createElement("h3");
    name.appendChild(document.createTextNode(cardList[idx]["name"]));
    const set = document.createElement("h4");
    set.appendChild(document.createTextNode(cardList[idx]["set"]));
