...
function setHeight() {
  ...
  var cardTextH = tempHeight - 40;
  var flipCard = document.querySelectorAll(".flip-card");
  for (f = 0; f < flipCard.length; f++) {
    flipCard[f].style.height = tempHeight + "px";
    // #projects .content .flip-card-back p
    var p = flipCard[f].getElementsByTagName("p");
    p[0].style.height = (cardTextH - 40) + "px";
  }
} // end setHeight()


//var temp = document.querySelectorAll(".flip-card");
//temp[0].getElementsByClassName("back-text")[0].getElementsByTagName("h4")[0].offsetHeight